...
sub TC_stimulation_and_measurement {
	...
	if ( @$tcpar_Fault[0] =~ m/frontcrash/i ) {
		...
		PD_Device_configuration( 'clear', ['AB1FD'] ); # Manitoo can't create 2 fault at time, using PD to create fault unexpected
		S_wait_ms(2800);
...
sub TC_evaluation {
	...
	foreach (@Provetime_Obt_6_8_arr) {
		...
		S_teststep_detected( "Observed value after each 600ms: '$_' ", 'wait_2s_and' );
	}
