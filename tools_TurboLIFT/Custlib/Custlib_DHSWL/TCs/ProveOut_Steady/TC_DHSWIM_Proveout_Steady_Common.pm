...
sub TC_stimulation_and_measurement {
	...
	if ( $tcpar_Precondition eq 'Nofault' ) {
		S_w2rep("No need to create precondition");
		S_wait_ms(8000); # For case No fault, nees to wait 8s for ECU ON to read fault PD
	}
	else {
	...
	S_teststep( "Check state of 'tcpar_DHSWL_signal' from first message transmit from IGN on for 6s", 'AUTO_NBR', 'wait_6s_since' );    #measurement 3
...
sub TC_evaluation {
	...
	S_teststep_expected( "'$tcpar_DHSWL_signal' shall be in '$tcpar_ON_State' for '$tcpar_ON_time_ms' ms from IGN ON ", 'wait_6s_since' );    #evaluation 3
	foreach (@Provetime_Obt_0_6_arr) {
		EVAL_evaluate_value( "Observed value in '$tcpar_ON_State' from IGN ON :", $_, "==", $tcpar_ON_State );
		S_teststep_detected( "Observed value: '$_' ", 'wait_6s_since' );
	}

	S_teststep_expected( "'$tcpar_DHSWL_signal' shall be   '$tcpar_OFF_State' for '$tcpar_OFF_time_ms' ms.", 'wait_2s_and' );                 #evaluation 4
	foreach (@Provetime_Obt_6_8_arr) {
		...
		S_teststep_detected( "Observed value: '$_' ", 'wait_2s_and' );

	}
	S_teststep_expected( "'$tcpar_DHSWL_signal' is in '$tcpar_Real_State' after '$tcpar_ProveOut_Time' ms from IGN ON.", 'waits_05s_and' );    #evaluation 5
	foreach (@Provetime_Obt_8_arr) {
		...
		S_teststep_detected( "Observed value: '$_' ", 'waits_05s_and' );
	}
	return 1;
}
