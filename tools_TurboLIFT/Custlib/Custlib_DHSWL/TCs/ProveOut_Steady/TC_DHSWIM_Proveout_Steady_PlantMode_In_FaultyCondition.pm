...
sub TC_stimulation_and_measurement {
	...
	if ( $tcpar_Fault eq 'No fault' ) {
		S_w2rep(" No create fault condition");
		S_wait_ms(8000); # wait time for ECU ON
	}
	else {
		FM_createFault($tcpar_Fault);    #used for TSG4
	}
	S_wait_ms($tcpar_t_Qual);            #modify
	...
	if ( $tcpar_Plant_mode eq 'Mazda plant mode' ) {
		...
		S_wait_ms(8000);
		PD_ReadFaultMemory();    #check Plant mode fault
		S_wait_ms(1000);
	}
	else {
		PD_ECUlogin();
		PD_WriteMemoryByName( 'rb_sycg_PlantModes_dfst.rb_sycg_ActivePlantModes_au8(0)', [0b00000100] );    #Set ECU to BoschPlantMode 3
		LC_ECU_Reset();
	...
	S_teststep( "Reset ECU and check state of '$tcpar_DHSWL_signal' from first message transmit from IGN on for 6s", 'AUTO_NBR', 'reset_ecu_and' );    #measurement 4
	LC_ECU_Off();
	S_wait_ms('TIMER_ECU_OFF');
	if ( $tcpar_Voltage_2 eq 'LowVoltage' ) {
		LC_ECU_On('LowVoltage');
...
sub TC_evaluation {
	...
	if ( $tcpar_Plant_mode eq 'Mazda plant mode' ) {
		S_teststep_detected( "ECU is in Plant mode", 'set_ecu_to' );
	}
	...
	S_teststep_expected( "'$tcpar_DHSWL_signal' shall be in '$tcpar_ON_State' for '$tcpar_ON_time_ms' ms from IGN ON.", 'reset_ecu_and' );    #evaluation 4
	foreach (@Provetime_Obt_0_6_arr) {
		EVAL_evaluate_value( "Observed value in '$tcpar_ON_time_ms' from IGN ON :", $_, "==", $tcpar_ON_State );
		S_teststep_detected( "Observed value after each 600ms: '$_' ", 'reset_ecu_and' );
	}
