...
sub TC_stimulation_and_measurement {
	...
	S_teststep( "Check state of '$tcpar_DHSWL_signal' ", 'AUTO_NBR', 'check_state_of_A' );    #measurement 2                                                                            # 10%tolerence for fault
	foreach ( 1 .. 8 ) {
	...
	S_wait_ms($tcpar_t_Dequal);                                                               #tolerace fault dequalified

	S_teststep( "Check state of '$tcpar_DHSWL_signal'", 'AUTO_NBR', 'check_state_of_B' );     #measurement 4                                                                           #wait for new expected updated value
	foreach ( 1 .. 8 ) {
		S_w2rep( "Get the '$tcpar_DHSWL_signal' prove out in step $_\n", 'blue' );
	...
	# move #measurement 3 to here because ReadFaultMemory time may affect to timing check
	...
	$status_after = PD_get_fault_status( $fault_struct_after, $tcpar_Fault );
...
sub TC_evaluation {
	...
	foreach my $value (@Provetime_Obt_8_arr) {
		S_teststep_detected( "Observed value : '$value' ", 'check_state_of_A' );
		EVAL_evaluate_value( "Observed value :", $value, "==", 3 );
	}
	...
	S_teststep_expected( "'$tcpar_DHSWL_signal' changes from '0x3' to '0x0' within 120 ms from step 5", 'check_state_of_B' );     #evaluation 4
