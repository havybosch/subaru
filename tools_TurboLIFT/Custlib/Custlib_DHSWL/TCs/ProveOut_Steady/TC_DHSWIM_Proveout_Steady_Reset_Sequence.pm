...
sub TC_stimulation_and_measurement {
	...
	S_teststep( "Execute step 7 for 4 times.", 'AUTO_NBR', 'execute_step_5' );    #measurement 6
	LC_ECU_Off();
	S_wait_ms('TIMER_ECU_OFF');
	LC_ECU_On();
	S_wait_ms( $tcpar_ProveOut_Time + 300 );
	...
	S_teststep( "Execute step 11 for 5 times", 'AUTO_NBR', 'execute_step_11' );    #measurement 10
	LC_ECU_Off();
	S_wait_ms('TIMER_ECU_OFF');
	LC_ECU_On();
	S_wait_ms( $tcpar_ProveOut_Time + 300 );
	...
	foreach ( 1 .. 4 ) {
		...
		push( @Provetime_Obt_8_after_reset_4_arr, hex($Provetime_Obt_8_after_reset_5) );
		S_wait_ms(500);
	}
...
sub TC_evaluation {
	...
	S_teststep_expected( "'$tcpar_DHSWL_signal' shall be in '$tcpar_ON_State' for '$tcpar_ON_time_ms' from IGN ON ", 'wait_6s_since' );    #evaluation 2
	...
	if ( $tcpar_Fault_state2 eq 'Empty' ) {
		PD_check_fault_status( $fault_struct_after, $tcpar_Fault1, '0bxxxx0xx0' );
	}
	else {
	...
	S_teststep_expected( "'$tcpar_DHSWL_signal' is in '$tcpar_Real_State2' after '$tcpar_ProveOut_Time' ms from IGN ON.", 'check_state_of' );                        #evaluation 8
	...
	S_teststep_expected( "'$tcpar_DHSWL_signal' is in '$tcpar_Real_State2' after '$tcpar_ProveOut_Time' ms.", 'reset_ecu__check_B' );                                #evaluation 9
	foreach (@Provetime_Obt_8_after_reset_arr) {
		EVAL_evaluate_value( "Observed value in '$tcpar_Real_State2' from from from 8s from IGN ON :", $_, "==", $tcpar_Real_State2 );
		S_teststep_detected( "Observed value after each 500ms: '$_' ", 'reset_ecu__check_B' );
	}

	S_teststep_expected( "The output is expected as step 11 output for 5 times.", 'execute_step_11' );                                                               #evaluation 10
