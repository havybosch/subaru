#### TEST CASE MODULE
package TC_DHSWIM_NotOperationFlt_AbnorVolQualification;
...
our $HEADER = q$Header: TCs/TCs_Project/DHSWL/Fault_Handling/Not_Operation_Fault/TC_DHSWIM_NotOperationFlt_AbnorVolQualification.pm 1.2 2018/08/28 13:11:28ICT UUL1HC develop  $;
...
our $PURPOSE = "Check fault qualified under abnormal voltage";

#>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>> DOCUMENTATION >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>

=head1 TESTCASE MODULE

TC_DHSWIM_NotOperationFlt_AbnorVolQualification
...
3. Set Voltage to <Low_Vol>.

4. Check Fault recorder after <Rx_Msg_name> transmitted for <cycles_1> cycles after step 3.

5. Set Voltage to <Normal_Vol>.

6. Check Fault recorder after <Rx_Msg_name> transmitted for <cycles_2> cycles after step 4.
...
4. <Fault> is in <Flt_status_1>.

5.

6. <Fault> is in <Flt_status_2>.

7. <Fault> is in <Flt_status_3>.
...
sub TC_stimulation_and_measurement {
	...
	#4 Set voltage to Low voltage
	S_teststep("Set Voltage to '$tcpar_Low_Vol'.", 'AUTO_NBR');
		 LC_SetVoltage($tcpar_Low_Vol);
	
	#5 Check flt recorder
	...
	#6 Set voltage to Normal voltage	
...
sub TC_evaluation {
	...
	#8 eval Flt	
	S_teststep_expected("'$tcpar_Fault' is in status: '$tcpar_Flt_status_3' ('$FltStatusMask{$tcpar_Flt_status_3}').", 'check_fault_recorder_C');			#evaluation 4
