#### TEST CASE MODULE
package TC_DHSWIM_MismatchFlt_AbnorVolPendingQualification;
...
our $HEADER = q$Header: TCs/TCs_Project/DHSWL/Fault_Handling/Mismatch_Fault/TC_DHSWIM_MismatchFlt_AbnorVolPendingQualification.pm 1.3 2018/09/19 09:57:37ICT UUL1HC develop  $;
...
our $PURPOSE = "Check fault qualification pending";

#>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>> DOCUMENTATION >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>

=head1 TESTCASE MODULE

TC_DHSWIM_MismatchFlt_AbnorVolPendingQualification
...
1. Create <PreCondition> for ECU send message <Tx_Msg_name> at CAN ID: <Tx_Msg_ID> with signal <Tx_Sig_name>  is  <Tx_Sig_value > and wait <T_time>.

2. Send message <Rx_Msg_name> at CAN ID: <Rx_Msg_ID> with conflict signal <Rx_Sig_name> is  <Rx_Sig_value> for <cycles_1>.

3. Set voltage to <Low_Vol> immediately  and <T_time_2>.

4.Send message <Rx_Msg_name> immediately  at CAN ID:  <Rx_Msg_ID> with <Rx_Sig_name> is  <Rx_Sig_value_1> for <cycles_2> cycles.

5. Check Fault recorder.

6. Set voltage to <Normal_Vol>.

7. Send message <Rx_Msg_name> immediately  at CAN ID:  <Rx_Msg_ID> with <Rx_Sig_name> is  <Rx_Sig_value>.

8.Check Fault recorder after <Rx_Msg_name> transmitted for <cycles_3> cycles after step 7.

9.Check Fault recorder after <Rx_Msg_name> transmitted for <cycles_4> cycles after step 8.


I<B<Evaluation>>

1.<Tx_Msg_name> transmitted at ID <Tx_Msg_ID> with signal <Tx_Sig_name>  is  <Tx_Sig_value >.
...
8.<Fault> is <Flt_status_2>.

9. <Fault> is <Flt_status_3>.
...
=head2 PARAMETER EXAMPLES
	...
	purpose = 'check behavior of DHSWL mismatch  fault qualified in abnormal voltage condition'
	
	Rx_Msg_name = '_HEC_IllumStatus'
	Rx_Msg_ID = '0x9F'
	Rx_Sig_name = 'DHS_RIL_STAT'
	#Rx_Sig_value = '<Test Heading 2>'
	
	Tx_Msg_name = '_RCMStatusMessage '
	Tx_Msg_ID = '0x340'
	Tx_Sig_name = 'DHS_RILReq'
	#Tx_Sig_value = '<Test Heading 1>'
...
sub TC_set_parameters {
	...
	$tcpar_Low_Vol =  GEN_Read_mandatory_testcase_parameter( 'Low_Vol' );
	$tcpar_Normal_Vol =  GEN_Read_mandatory_testcase_parameter( 'Normal_Vol' );
...
sub TC_stimulation_and_measurement {
	...
	S_teststep("Create '$tcpar_PreCondition' for ECU send message '$tcpar_Tx_Msg_name' at CAN ID: '$tcpar_Tx_Msg_ID' with signal '$tcpar_Tx_Sig_name'  is  '$tcpar_Tx_Sig_value ' and wait '$tcpar_T_time' ms.", 'AUTO_NBR', 'create_precondition_for');			#measurement 1
	...
	S_teststep("Send message '$tcpar_Rx_Msg_name' at CAN ID: '$tcpar_Rx_Msg_ID' with conflict signal '$tcpar_Rx_Sig_name' is  '$tcpar_Rx_Sig_value' for '$tcpar_cycles_1' cycles.", 'AUTO_NBR');
		COM_setSignalState($tcpar_Rx_Sig_name, $RxLampState{$tcpar_Rx_Sig_value});
		S_wait_ms( $tcpar_cycles_1 * $RxMsgCyclicTime -180);
		
	S_teststep("Set voltage to '$tcpar_Low_Vol' immediately  and '$tcpar_T_time_2' ms.", 'AUTO_NBR');
		LC_SetVoltage($tcpar_Low_Vol);
		 S_wait_ms($tcpar_T_time_2);
		 
	S_teststep("Send message '$tcpar_Rx_Msg_name' immediately  at CAN ID:  '$tcpar_Rx_Msg_ID' with '$tcpar_Rx_Sig_name' is  '$tcpar_Rx_Sig_value_1' for '$tcpar_cycles_2' cycles.", 'AUTO_NBR');
		COM_setSignalState($tcpar_Rx_Sig_name, $RxLampState{$tcpar_Rx_Sig_value_1});
		S_wait_ms( $tcpar_cycles_2 * $RxMsgCyclicTime );
		
	S_teststep("Check Fault recorder.", 'AUTO_NBR', 'check_fault_recorder_A');			#measurement 2
		$flt_struct_1 = PD_ReadFaultMemory();
		
	S_teststep("Set voltage to '$tcpar_Normal_Vol'.", 'AUTO_NBR');
		LC_SetVoltage($tcpar_Normal_Vol);
		
	S_teststep("Send message '$tcpar_Rx_Msg_name' immediately  at CAN ID:  '$tcpar_Rx_Msg_ID' with '$tcpar_Rx_Sig_name' is  '$tcpar_Rx_Sig_value'.", 'AUTO_NBR');
		COM_setSignalState($tcpar_Rx_Sig_name, $RxLampState{$tcpar_Rx_Sig_value});
		
	S_teststep("Check Fault recorder after '$tcpar_Rx_Msg_name' transmitted for '$tcpar_cycles_3' cycles after step 7.", 'AUTO_NBR', 'check_fault_recorder_B');			#measurement 3
		S_wait_ms( $tcpar_cycles_3 * $RxMsgCyclicTime );		
		$flt_struct_2 = PD_ReadFaultMemory();
		
	S_teststep("Check Fault recorder after '$tcpar_Rx_Msg_name' transmitted for '$tcpar_cycles_4' cycles after step 8.", 'AUTO_NBR', 'check_fault_recorder_C');			#measurement 4
		S_wait_ms( $tcpar_cycles_4 * $RxMsgCyclicTime );
...
sub TC_evaluation {
	...
	#1 Eval signal
	S_teststep_expected("'$tcpar_Tx_Msg_name' transmitted at ID '$tcpar_Tx_Msg_ID' with signal '$tcpar_Tx_Sig_name'  is  '$tcpar_Tx_Sig_value '.", 'create_precondition_for');			#evaluation 1
	S_teststep_detected("'$tcpar_Tx_Sig_name' is '$TxRILReqSignal'", 'create_precondition_for');
		EVAL_evaluate_value( "DHS_RILReq signal", $TxRILReqSignal, '==', $tcpar_Tx_Sig_value );
		
	#4 Eval Flt	
	S_teststep_expected("'$tcpar_Fault' is status '$tcpar_Flt_status_1' ('$FltStatusMask{$tcpar_Flt_status_1}').", 'check_fault_recorder_A');			#evaluation 2
	...
	#7 Eval Flt	
	S_teststep_expected("'$tcpar_Fault' is status '$tcpar_Flt_status_2'  ('$FltStatusMask{$tcpar_Flt_status_2}').", 'check_fault_recorder_B');			#evaluation 3
	...
	S_teststep_expected("'$tcpar_Fault' is status '$tcpar_Flt_status_3'  ('$FltStatusMask{$tcpar_Flt_status_3}').", 'check_fault_recorder_C');			#evaluation 4
