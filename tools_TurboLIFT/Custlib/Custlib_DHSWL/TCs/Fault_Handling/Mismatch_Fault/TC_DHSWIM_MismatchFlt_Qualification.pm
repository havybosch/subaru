#### TEST CASE MODULE
package TC_DHSWIM_MismatchFlt_Qualification;
...
our $HEADER = q$Header: TCs/TCs_Project/DHSWL/Fault_Handling/Mismatch_Fault/TC_DHSWIM_MismatchFlt_Qualification.pm 1.3 2018/09/18 15:59:22ICT UUL1HC develop  $;
...
TC_DHSWIM_MismatchFlt_Qualification
...
2. Send message <Rx_Msg_name> at CAN ID: <Rx_Msg_ID> with <Rx_Sig_name> is  <Rx_Sig_value>.

3. Check Fault recorder after <Rx_Msg_name> transmitted for <cycles_1> cycles after step 2.

4.Check Fault recorder after <Rx_Msg_name> transmitted for <cycles_2> cycles after step 3.
...
=head2 PARAMETER EXAMPLES
	...
	purpose = 'check behavior of DHSWL Mismatch fault qualified'
...
sub TC_initialization {
	...
	#Set init value of DHS_RIL_Stat for Flt free
...
sub TC_stimulation_and_measurement {
	...
	S_teststep("Create '$tcpar_PreCondition' to ECU send message '$tcpar_Tx_Msg_name' at CAN ID: '$tcpar_Tx_Msg_ID' with signal '$tcpar_Tx_Sig_name'  is  '$tcpar_Tx_Sig_value ' and wait '$tcpar_T_time' cycles.", 'AUTO_NBR', 'create_precondition_to');			#measurement 1
		...
		else{S_wait_ms($tcpar_T_time+250);}
		
		#Read value of signal RILReq
		($TxRILReqSignal) = CA_read_can_signal( $tcpar_Tx_Msg_name.'::'.$tcpar_Tx_Sig_name, 'hex' );
		$TxRILReqSignal =~ s/0x//;
		
		#"Check Fault recoder before set signal"
		PD_ReadFaultMemory();
	#2 Send message _HEC with invalid signal RIL_STAT	
	S_teststep("Send message '$tcpar_Rx_Msg_name' at CAN ID: '$tcpar_Rx_Msg_ID' with '$tcpar_Rx_Sig_name' is  '$tcpar_Rx_Sig_value'.", 'AUTO_NBR');
		COM_setSignalState($tcpar_Rx_Sig_name, $RxLampState{$tcpar_Rx_Sig_value});
	
	#3 Check Flt	
	S_teststep("Check Fault recorder after '$tcpar_Rx_Msg_name' transmitted for '$tcpar_cycles_1' cycles after step 2.", 'AUTO_NBR', 'check_fault_recorder_A');			#measurement 2
		S_wait_ms( $tcpar_cycles_1 * $RxMsgCyclicTime -180);
		COM_stopMessages($tcpar_Rx_Msg_name);
		$flt_struct_1 = PD_ReadFaultMemory();

	#4 Check Flt	
	S_teststep("Check Fault recorder after '$tcpar_Rx_Msg_name' transmitted for '$tcpar_cycles_2' cycles after step 3", 'AUTO_NBR', 'check_fault_recorder_B');			#measurement 3
...
sub TC_evaluation {
	...
	S_teststep_expected("'$tcpar_Tx_Msg_name' transmitted at ID '$tcpar_Tx_Msg_ID' with signal '$tcpar_Tx_Sig_name'  is  '$tcpar_Tx_Sig_value '.", 'create_precondition_to');			#evaluation 1
	S_teststep_detected("'$tcpar_Tx_Sig_name' is '$TxRILReqSignal'", 'create_precondition_to');
