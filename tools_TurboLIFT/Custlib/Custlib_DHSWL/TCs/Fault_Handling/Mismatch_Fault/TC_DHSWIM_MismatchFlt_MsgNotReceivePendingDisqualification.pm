#### TEST CASE MODULE
package TC_DHSWIM_MismatchFlt_MsgNotReceivePendingDisqualification;
...
our $HEADER = q$Header: TCs/TCs_Project/DHSWL/Fault_Handling/Mismatch_Fault/TC_DHSWIM_MismatchFlt_MsgNotReceivePendingDisqualification.pm 1.3 2018/09/21 11:31:42ICT UUL1HC develop  $;
...
TC_DHSWIM_MismatchFlt_MsgNotReceivePendingDisqualification
...
3. Create <Condition> and wait <T_time>.

4. Check Fault recorder.

5. Remove <Condition>.

6. Send  message <Rx_Msg_name> at CAN ID: <Rx_Msg_ID> with valid signal<Rx_Sig_name> is  <Rx_Sig_value_2> after step 5 .

7. Check Fault recorder after <Rx_Msg_name> transmitted for <cycles_2> cycles after step 6.
...
=head2 PARAMETER EXAMPLES
	...
	purpose = 'check behavior of DHSWL Mismatch fault pending Disqualified in abnormal condition'
...
sub TC_stimulation_and_measurement {
	...
	#3 Create condition	
	S_teststep("Create '$tcpar_Condition' and wait '$tcpar_T_time' ms.", 'AUTO_NBR');
	...
	S_teststep("Check Fault recorder.", 'AUTO_NBR', 'check_fault_recorder_A');			#measurement 2
		if($tcpar_Condition eq 'Busoff'){
			#Do nothing, bc can not read fault by PD during busoff
		}
		else{
	...
	#6 Send message _HEC with valid signal RIL_STAT	
	S_teststep("Send  message '$tcpar_Rx_Msg_name' at CAN ID: '$tcpar_Rx_Msg_ID' with valid signal '$tcpar_Rx_Sig_name' is  '$tcpar_Rx_Sig_value_2' after step 5 .", 'AUTO_NBR');
