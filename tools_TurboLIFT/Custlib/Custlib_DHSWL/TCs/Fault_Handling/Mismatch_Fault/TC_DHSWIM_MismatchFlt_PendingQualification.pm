#### TEST CASE MODULE
package TC_DHSWIM_MismatchFlt_PendingQualification;
...
our $HEADER = q$Header: TCs/TCs_Project/DHSWL/Fault_Handling/Mismatch_Fault/TC_DHSWIM_MismatchFlt_PendingQualification.pm 1.3 2018/09/18 16:01:06ICT UUL1HC develop  $;
...
TC_DHSWIM_MismatchFlt_PendingQualification
...
2. Send message <Rx_Msg_name> at CAN ID: <Rx_Msg_ID> with conflict signal <Rx_Sig_name> is  <Rx_Sig_value_1> for <cycles_1>.

3. Send message <Rx_Msg_name> at  CAN ID: <Rx_Msg_ID> with non-Mismatch signal<Rx_Sig_name> is  <Rx_Sig_value_2> for <cycles_2>.

4. Check Fault recorder.

5. Send message <Rx_Msg_name> immediately  at CAN ID:  <Rx_Msg_ID> with conflict signal <Rx_Sig_name> is  <Rx_Sig_value_1>.

6.Check Fault recorder after <Rx_Msg_name> transmitted for <cycles_3> cycles after step 5.

7.Check Fault recorder after <Rx_Msg_name> transmitted for <cycles_4> cycles after step 6.
...
=head2 PARAMETER EXAMPLES
	...
	purpose = 'check behavior of DHSWL Mismatch fault reset/pending qualified with non-mismatch condition'
	
	Rx_Msg_name = '_HEC_IllumStatus'
	Rx_Msg_ID = '0x9F'
	Rx_Sig_name = 'DHS_RIL_STAT'
	#Rx_Sig_value_1 = '<Test Heading 2>'
	#Rx_Sig_value_2 = '<Test Heading 3>'
...
sub TC_stimulation_and_measurement {
	...
	S_teststep("Create '$tcpar_PreCondition'  to  ECU send message '$tcpar_Tx_Msg_name' at CAN ID: '$tcpar_Tx_Msg_ID' with signal '$tcpar_Tx_Sig_name'  is  '$tcpar_Tx_Sig_value ' and wait '$tcpar_T_time' ms.", 'AUTO_NBR', 'create_precondition__to');			#measurement 1
	...
	S_teststep("Send message '$tcpar_Rx_Msg_name' at CAN ID: '$tcpar_Rx_Msg_ID' with conflict signal '$tcpar_Rx_Sig_name' is  '$tcpar_Rx_Sig_value_1' for '$tcpar_cycles_1' cycles.", 'AUTO_NBR');
		COM_setSignalState($tcpar_Rx_Sig_name, $RxLampState{$tcpar_Rx_Sig_value_1} );
		S_wait_ms( $tcpar_cycles_1 * $RxMsgCyclicTime -180);
		
	#3 Send message _HEC with valid signal RIL_STAT		
	S_teststep("Send message '$tcpar_Rx_Msg_name' at  CAN ID: '$tcpar_Rx_Msg_ID' with non-Mismatch signal'$tcpar_Rx_Sig_name' is  '$tcpar_Rx_Sig_value_2' for '$tcpar_cycles_2'cycles.", 'AUTO_NBR');
	...
	#5 Send message _HEC with invalid signal RIL_STAT		
	S_teststep("Send message '$tcpar_Rx_Msg_name' immediately  at CAN ID:  '$tcpar_Rx_Msg_ID' with conflict signal '$tcpar_Rx_Sig_name' is  '$tcpar_Rx_Sig_value_1'.", 'AUTO_NBR');
		COM_setSignalState($tcpar_Rx_Sig_name, $RxLampState{$tcpar_Rx_Sig_value_1});
	
	#6 Check flt	
	S_teststep("Check Fault recorder after '$tcpar_Rx_Msg_name' transmitted for '$tcpar_cycles_3' cycles after step 5.", 'AUTO_NBR', 'check_fault_recorder_B');			#measurement 3
		S_wait_ms( $tcpar_cycles_3 * $RxMsgCyclicTime -180 );
		$flt_struct_2 = PD_ReadFaultMemory();
		
	#7 Check flt
	S_teststep("Check Fault recorder after '$tcpar_Rx_Msg_name' transmitted for '$tcpar_cycles_4' cycles after step 6.", 'AUTO_NBR', 'check_fault_recorder_C');			#measurement 4
...
sub TC_evaluation {
	#1 EVAL TX signal value 
	S_teststep_expected("'$tcpar_Tx_Msg_name' transmitted at ID '$tcpar_Tx_Msg_ID' with signal '$tcpar_Tx_Sig_name'  is  '$tcpar_Tx_Sig_value '.", 'create_precondition__to');			#evaluation 1
	S_teststep_detected("'$tcpar_Tx_Sig_name' is '$TxRILReqSignal'", 'create_precondition__to');
		EVAL_evaluate_value( "RILReq", $TxRILReqSignal, '==', $tcpar_Tx_Sig_value );
