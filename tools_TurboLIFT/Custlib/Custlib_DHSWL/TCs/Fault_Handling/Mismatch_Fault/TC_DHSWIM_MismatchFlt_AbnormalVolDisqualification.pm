#### TEST CASE MODULE
package TC_DHSWIM_MismatchFlt_AbnormalVolDisqualification;
...
our $HEADER = q$Header: TCs/TCs_Project/DHSWL/Fault_Handling/Mismatch_Fault/TC_DHSWIM_MismatchFlt_AbnormalVolDisqualification.pm 1.2 2018/08/28 13:11:27ICT UUL1HC develop  $;
...
our $PURPOSE = "Check mismatch fault Disqualification under abnormal Voltage";

#>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>> DOCUMENTATION >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>

=head1 TESTCASE MODULE

TC_DHSWIM_MismatchFlt_AbnormalVolDisqualification
...
1. Create <PreCondition> to ECU send message <Tx_Msg_name> at CAN ID: <Tx_Msg_ID> with signal <Tx_Sig_name>  is  <Tx_Sig_value > and wait <T_time> ms.
...
3.Set voltage to <Low_Vol> and wait <T_time_2> ms.

4. Check Fault recorder.

5. Send  message <Rx_Msg_name> at CAN ID: <Rx_Msg_ID> with valid signal <Rx_Sig_name> is  <Rx_Sig_value_2> after step 4 .

 

6. Check Fault recorder after message  <Rx_Msg_name> transmitted for <cycles_2> cycles.

7. Set voltage to <Normal_Vol>.

8. Check Fault recorder after <Rx_Msg_name> transmitted for <cycles_2> cycles after step 6.
...
=head2 PARAMETER EXAMPLES
	...
	purpose = 'check behavior of DHSWL Mismatch fault disqualified in abnormal condition'
	
	Rx_Msg_name = '_HEC_IllumStatus'
	Rx_Msg_ID = '0x9F'
	Rx_Sig_name = 'DHS_RIL_STAT'
	Rx_Sig_value_1 = '6'#RxInvalid1
	#Rx_Sig_value_2 = '<Test Heading 2>'
...
sub TC_stimulation_and_measurement {
	...
	S_teststep("Create Fault '$tcpar_Fault' by Send message '$tcpar_Rx_Msg_name' at CAN ID: '$tcpar_Rx_Msg_ID' with conflict signal '$tcpar_Rx_Sig_name' is  '$tcpar_Rx_Sig_value_1' for '$tcpar_cycles_1' cycles .", 'AUTO_NBR', 'create_fault_fault');			#measurement 1
		COM_setSignalState($tcpar_Rx_Sig_name, $RxLampState{$tcpar_Rx_Sig_value_1});
		S_wait_ms( $tcpar_cycles_1 * $RxMsgCyclicTime );
	
	#3 Set Voltage to Low	
	S_teststep("Set voltage to '$tcpar_Low_Vol' and wait '$tcpar_T_time_2' ms.", 'AUTO_NBR');
	...
	#5 Send message _HEC with valid signal RIL_STAT
	S_teststep("Send  message '$tcpar_Rx_Msg_name' at CAN ID: '$tcpar_Rx_Msg_ID' with valid signal '$tcpar_Rx_Sig_name' is  '$tcpar_Rx_Sig_value_2' after step 4 .", 'AUTO_NBR');
		COM_setSignalState($tcpar_Rx_Sig_name, $RxLampState{$tcpar_Rx_Sig_value_2});
		
	#6 Check Flt		
	S_teststep("Check Fault recorder after message  '$tcpar_Rx_Msg_name' transmitted for '$tcpar_cycles_2' cycles.", 'AUTO_NBR');
		S_wait_ms($tcpar_cycles_2*$RxMsgCyclicTime);
		$flt_struct_2 = PD_ReadFaultMemory();
		
	#7 Set Voltage to Normal 		
	S_teststep("Set voltage to '$tcpar_Normal_Vol'.", 'AUTO_NBR');
		LC_SetVoltage($tcpar_Normal_Vol);
	
	#8 Check Flt
	S_teststep("Check Fault recorder after '$tcpar_Rx_Msg_name' transmitted for '$tcpar_cycles_3' cycles after step 6.", 'AUTO_NBR', 'check_fault_recorder_B');			#measurement 3
...
sub TC_evaluation {
	...
	S_teststep_expected("'$tcpar_Fault' is status '$tcpar_Flt_status_1' ('$FltStatusMask{$tcpar_Flt_status_1}').", 'create_fault_fault');			#evaluation 1
		$faultStatus_1 = PD_get_fault_status( $flt_struct_1, $tcpar_Fault );
		my $temp1 = sprintf( "0b%08b", hex($faultStatus_1) ); 
	S_teststep_detected("'$tcpar_Fault' status is '$temp1'", 'create_fault_fault');
		PD_check_fault_status( $flt_struct_1, $tcpar_Fault, $FltStatusMask{$tcpar_Flt_status_1} ); 	
		
	S_teststep_expected("'$tcpar_Fault' is status '$tcpar_Flt_status_2' ('$FltStatusMask{$tcpar_Flt_status_2}').", 'check_fault_recorder_A');			#evaluation 2
		$faultStatus_2 = PD_get_fault_status( $flt_struct_2, $tcpar_Fault );
		my $temp2 = sprintf( "0b%08b", hex($faultStatus_2) ); 
	S_teststep_detected("'$tcpar_Fault' status is '$temp2'", 'check_fault_recorder_A');
		PD_check_fault_status( $flt_struct_2, $tcpar_Fault, $FltStatusMask{$tcpar_Flt_status_2} );
		
	S_teststep_expected("'$tcpar_Fault' is status '$tcpar_Flt_status_3' ('$FltStatusMask{$tcpar_Flt_status_3}').", 'check_fault_recorder_B');			#evaluation 3
		$faultStatus_3 = PD_get_fault_status( $flt_struct_3, $tcpar_Fault );
		my $temp3 = sprintf( "0b%08b", hex($faultStatus_3) ); 
	S_teststep_detected("'$tcpar_Fault' status is '$temp3'", 'check_fault_recorder_B');
