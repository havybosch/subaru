#### TEST CASE MODULE
package TC_OEMEDID_P_SeatInformation;
...
our $HEADER = q$Header: TCs_CP_Y/TCs_Project/EDR_TR8/TC_OEMEDID_P_SeatInformation.pm 1.1 2019/07/05 06:59:13ICT Kobayashi Kohji (CC-OSS/ENG3-JP) (khk3to) develop RNO1HC(2019/10/29 15:22:06ICT) $;
...
our $PURPOSE = "To check the Passenger seat information during crash";

#>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>> DOCUMENTATION >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>

=head1 TESTCASE MODULE

TC_OEMEDID_P-SeatInformation
...
=head2 PARAMETER EXAMPLES
	...
	purpose		 = 'to check the Passenger seat information during crash'
...
sub TC_stimulation_and_measurement {
   ...
   S_user_action ( "Set the Occupant status in the PODS simulator as '$tcpar_Occupant_value'" );
	...
	S_teststep("Read '$tcpar_EDID' in EDR1 and EDR2 memory.", 'AUTO_NBR');	
...
sub TC_evaluation {
	...
	for( $recordNumber = 1; $recordNumber <= $edrNumberOfEventsToBeStored; $recordNumber++)
	{
