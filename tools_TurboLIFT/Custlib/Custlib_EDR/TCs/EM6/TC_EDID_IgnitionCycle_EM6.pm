#### TEST CASE MODULE
package TC_EDID_IgnitionCycle_EM6;
...
0. if<POC> != NA, set NVM POC to <POC>
...
8. Read <EDID> value in the corresponding EDR 

9. Set signal <IGN_Variable> to <Value_Signal2>

10. Note down the Value of <Value_Signal>

11. Inject <CrashType3> 

12. Read <EDID> value in the corresponding EDR
...
6. IgnitonCycleCrash should have value same as Ignition Cycle as recorded in during Crash

7. -

8. IgnitonCycleCrash should have value same as Ignition Cycle as recorded in during Crash(Incremented by 1 value compared to step 3)
...
12. IgnitonCycleCrash should have value same as Ignition Cycle as recorded in during Crash(Incremented by NumberOfECU_Reset_before_2rdCrash value compared to step 6)
...
=head2 PARAMETER EXAMPLES
	...
	purpose	= 'To validate Ignition Cycle Crash recorded in EDR'
...
#2.	check value of signal 
	# if signal  error => expected_value = PRD_READMEMORY();
	# else send value 
# #3.	Inject crash
# 4.	trigger_msg
# 5. _readandStored
...
###################################	
	...
	if( $tcpar_POC ne "NA")
	{
		...
		PD_WriteMemoryByName("rb_tim_EcuOnTimeDataEe_st.POnCounter_u32",$tcpar_POC);
		S_wait_ms(500);
	}
...
############# CRASH 2#################	
	...
	# Read_and_stored($tcpar_CrashType2,"$tcpar_CrashType1");
...
############ CRASH 3#################
	...
	# Read_and_stored($tcpar_CrashType3,"$tcpar_CrashType1");
	return 1;
}
...
sub Evaluate_each_case{
    ...
    # my $edidData = $record_handler -> GetDecodedEDID("CrashLabel" =>$tcpar_CrashType, "RecordNumber" => 1,"EDIDnr" => $tcpar_EDID );
