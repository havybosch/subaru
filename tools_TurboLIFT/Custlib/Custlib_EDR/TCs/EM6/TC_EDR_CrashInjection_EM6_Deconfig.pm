#### TEST CASE MODULE
package TC_EDR_CrashInjection_EM6_Deconfig;
...
my $tcpar_value_min;
my $tcpar_SquibLabel;
my $tcpar_Crashtype_select;
my $tcpar_CrashTimeZero_ms_List;
...
#-------------------------------------------------------------------------------
    ...
    @tcpar_CrashtypeList = GEN_Read_mandatory_testcase_parameter('crashtype');
	...
	$tcpar_CrashTimeZero_ms_List = S_read_optional_testcase_parameter('CrashTimeZero_ms', 'byref');
	...
	$tcpar_State = S_read_optional_testcase_parameter('State');
...
# Read fault memory after crash
    ...
    PD_ReadFaultMemory();   
#-------------------------------------------------------------------------------
	# if($tcpar_DriverSide eq 'Left') {
			# #####Config Left/Right Driver###
		# }else{
		
		# }
		# S_teststep_2nd_level("Config driver side", 'AUTO_NBR');
		# if($tcpar_State eq 'Deconfig') {
			# ####Deconfig Squibs####
		# }else{
...
sub TC_stimulation_and_measurement {
	# S_w2rep("Crash : '@tcpar_CrashtypeList'");
	foreach	my $Crashtype (@tcpar_CrashtypeList){
		
		$tcpar_CrashTimeZero_ms = $tcpar_CrashTimeZero_ms_List->{$Crashtype};
		...
		my $tcpar_Crashtype = $Crashtype;
		$tcpar_Crashtype_select = $tcpar_Crashtype;
		S_w2rep("Inject crash ".$Crashtype);
		...
		S_teststep("Config driver side $tcpar_DriverSide, $tcpar_State with $tcpar_SquibLabel", 'AUTO_NBR');
		S_teststep_2nd_level("Config driver side", 'AUTO_NBR');
		...
		my $dataStoragePath = "$main::REPORT_PATH/".$tcpar_Crashtype."_".$tcpar_SquibLabel."_".$tcpar_State."_".$tcpar_DriverSide;
		...
		if (lc($tcpar_read_CHINAEDR) eq 'yes'){
			...
			EDR_ReadAndStoreAllRecords_Subaru ("DiagType" => $ChinaEDR_diagType,
									"CrashLabel" => $tcpar_Crashtype."_".$tcpar_SquibLabel."_".$tcpar_State."_".$tcpar_DriverSide,
									"NbrOfRecords" =>  $numberOfRecords,
									"StoragePath" => $dataStoragePath,
									"CrashInfo" => $crashInfo_href,
									"read_EDRType"=>'CHINA');
	...
	# # poh3hc
		...
		my $recordAvailable = $record_handler -> IsRecordAvailable("CrashLabel" => $tcpar_Crashtype."_".$tcpar_SquibLabel."_".$tcpar_State."_".$tcpar_DriverSide, "RecordNumber"=> 1);
		if($recordAvailable){
			$record_handler -> ExportCrashRecordsToExcel("CrashLabel" => $tcpar_Crashtype."_".$tcpar_SquibLabel."_".$tcpar_State."_".$tcpar_DriverSide,
														"Filepath" => $dataStoragePath."\\".$tcpar_Crashtype."_".$tcpar_SquibLabel."_".$tcpar_State."_".$tcpar_DriverSide.".xlsx");		
		}
		...
		foreach my $recordNbr (1..$numberOfRecords)
			{
				...
				#    
				# ----------------------------------------------------------- 
				my $recordAvailable = $record_handler -> IsRecordAvailable("CrashLabel" => $tcpar_Crashtype."_".$tcpar_SquibLabel."_".$tcpar_State."_".$tcpar_DriverSide, "RecordNumber"=> $recordNbr);
		...
		if($tcpar_StorageOptions_href->{'SwitchStates'} eq 'yes')
		{
			...
			foreach my $environmentDevice (keys %{$crashRawSignals -> {'ENVIRONMENT'}})
			{
				...
				$crash_handler -> AddCrashSource(  "CrashLabel" => $tcpar_Crashtype."_".$tcpar_SquibLabel."_".$tcpar_State."_".$tcpar_DriverSide,
												   "SourceType" => 'Switch', 
												   "SourceLabel" => $switch,
		...
		######################Final Session################
		S_teststep_2nd_level("Config driver side", 'AUTO_NBR');
		if($tcpar_SquibLabel eq 'Deconfig') {
			####Deconfig Squibs####
		}else{
		...
		if(S_get_exec_option_NOERROR ( 'EDR_CrashInjection_DeleteRecordObjects' )){
			...
			if(not defined $numberOfRecords){
				...
				foreach my $recordNbr (1..$numberOfRecords)
				{
					$record_handler -> DeleteRecord("CrashLabel" => $tcpar_Crashtype."_".$tcpar_SquibLabel."_".$tcpar_State."_".$tcpar_DriverSide, "RecordNumber" => $recordNbr);
					$record_handler -> DeleteRecord("CrashLabel" => $tcpar_Crashtype."_".$tcpar_SquibLabel."_".$tcpar_State."_".$tcpar_DriverSide."CHINA", "RecordNumber" => $recordNbr);
					$record_handler -> DeleteRecord("CrashLabel" => $tcpar_Crashtype."_".$tcpar_SquibLabel."_".$tcpar_State."_".$tcpar_DriverSide."_Supplier", "RecordNumber" => $recordNbr);
				}                
			}
	...
	my $numberOfRecords =shift; 
	 ...
	 if(S_get_exec_option_NOERROR ( 'EDR_CrashInjection_DeleteRecordObjects' )){
        ...
        if(not defined $numberOfRecords){
            ...
            foreach my $recordNbr (1..$numberOfRecords)
            {
                $record_handler -> DeleteRecord("CrashLabel" => $tcpar_Crashtype_select, "RecordNumber" => $recordNbr);
				$record_handler -> DeleteRecord("CrashLabel" => $tcpar_Crashtype_select."CHINA", "RecordNumber" => $recordNbr);
                $record_handler -> DeleteRecord("CrashLabel" => $tcpar_Crashtype_select."_Supplier", "RecordNumber" => $recordNbr);
            }                
        }
