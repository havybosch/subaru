#### TEST CASE MODULE
package TC_EDID_Simple_Validation_Quanhh_Multi_VIN;
...
=head2 PARAMETER NAMES
	...
	SCALAR 'TC purpose: Validate EDID related to ECU HW number, ECU serial number, ECU SW number.' => 
	SCALAR 'Stimulation:' => 
	SCALAR 'Inject Crash and read EDID corresponding to ECU information' => 
	SCALAR 'Evaluation:' => 
	SCALAR 'EDID related to ECU information should have value as per information of the ECU' => 
	SCALAR 'Notes:' => 
	SCALAR 'purpose' => 
...
=head2 PARAMETER EXAMPLES
	...
	TC purpose: Validate EDID related to ECU HW number, ECU serial number, ECU SW number.
	
	Stimulation: 
	Inject Crash and read EDID corresponding to ECU information
	
	Evaluation: 
	EDID related to ECU information should have value as per information of the ECU
...
my $tcpar_TC; 
# purpose: Validate EDID related to ECU HW number, ECU serial number, ECU SW number.;
my $tcpar_Stimulation;
my $tcpar_InjectCrash;
 # and read EDID corresponding to ECU information;
my $tcpar_Evaluation;
my $tcpar_EDID; 
my $tcpar_purpose;
my $tcpar_read_CHINAEDR;
...
my $default_VIN = "2E F1 90 00 00 00 00 00 00 00 31 00 00 00 00 00 00 00 00 00";
...
sub TC_set_parameters {
	...
	$tcpar_Crashcode1 =  S_read_optional_testcase_parameter( 'Crashcode1');
	$tcpar_Crashcode2=  S_read_optional_testcase_parameter( 'Crashcode2');
	$tcpar_Crashcode3 =  S_read_optional_testcase_parameter( 'Crashcode3');
	...
	# if ((not defined $tcpar_ExpectedValue_Decoded) and (not defined $tcpar_ExpectedValue_Raw)){
		...
		# return 1;
	# }
	$tcpar_VIN_Value1 =  S_read_optional_testcase_parameter( 'VIN_Value1' );
	$tcpar_VIN_Value2 =  S_read_optional_testcase_parameter( 'VIN_Value2' );
	$tcpar_VIN_Value3 =  S_read_optional_testcase_parameter( 'VIN_Value3' );
...
sub TC_stimulation_and_measurement {
	...
	$Expected_VIN1 = write_VIN($tcpar_VIN_Value1);
	S_teststep("Inject a Crash '$tcpar_Crashcode1'", 'AUTO_NBR');
	Crash_Injection($tcpar_Crashcode1);
	S_wait_ms(10000);
	
	Prepertation();
	
	# S_teststep("Read EDR record 1st", 'AUTO_NBR');			#measurement 3
	# ReadAndStoreRecord($tcpar_Crashcode1);
	S_wait_ms(500);
#######################################################################	
	Prepertation();
	# WRITE VIN
	$Expected_VIN2 = write_VIN($tcpar_VIN_Value2);
	S_teststep("Inject a Crash '$tcpar_Crashcode2'", 'AUTO_NBR');
	Crash_Injection($tcpar_Crashcode2);
	S_wait_ms(10000);
	
	Prepertation();
	
	# S_teststep("Read EDR record 2nd", 'AUTO_NBR');			#measurement 3
	# ReadAndStoreRecord($tcpar_Crashcode2);

###########################################################################
	Prepertation();
	# WRITE VIN
	$Expected_VIN3 = write_VIN($tcpar_VIN_Value3);
	S_teststep("Inject a Crash '$tcpar_Crashcode3'", 'AUTO_NBR');
	Crash_Injection($tcpar_Crashcode3);
	S_wait_ms(10000);
	
	Prepertation();
	
	# S_teststep("Read EDR record 3rd", 'AUTO_NBR');			#measurement 3
...
sub TC_evaluation {
	...
	SystemEDID_Evaluate(3,$tcpar_EDID,$Expected_VIN1,$tcpar_Crashcode3);
	SystemEDID_Evaluate(2,$tcpar_EDID,$Expected_VIN2,$tcpar_Crashcode3);
	SystemEDID_Evaluate(1,$tcpar_EDID,$Expected_VIN3,$tcpar_Crashcode3);
	return 1;
}
...
foreach my $recordNbr (1..$tcpar_ExpectedNbrOfRecords)
	{
		...
		$record_handler -> DeleteAllRecords("CrashLabel" => "$tcpar_Crashcode3", "RecordNumber" => $recordNbr);
	...
	# my @crash_label_array = ( $tcpar_CrashPrecondition1,$tcpar_CrashPrecondition2,$tcpar_CrashPrecondition3,$tcpar_Crashcode1,$tcpar_Crashcode2,$tcpar_Crashcode3 );                                     
		my @crash_label_array = ( $tcpar_Crashcode1,$tcpar_Crashcode2,$tcpar_Crashcode3 );                                     
...
sub write_VIN{
	my $data_VIN = shift;
	# my $Crash_code = shift;
	S_teststep("Send request 'WriteDataByIdentifier_VIN' with '$data_VIN'.", 'AUTO_NBR');			
	GDCOM_request('2E F1 90 '.$data_VIN,'6E F1 90', 'relax');
	S_wait_ms(500);
	...
	# Crash_Injection($Crash_code);
	# S_wait_ms(10000);
	
	# Prepertation();
	
	# S_teststep("Read EDR record", 'AUTO_NBR');			#measurement 3
	# ReadAndStoreRecord($Crash_code);
	# S_wait_ms(500);
...
#-------------------------------------------------------------------------------
    ...
    my $detected_Value_RawHex_Projectaref = $record_handler -> GetRawEDID(
																			"EDIDnr" => $edid,
	...
	##Quanhh print 
	S_w2rep("Quanhh print $detected_Value_RawHex");
