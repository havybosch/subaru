#### TEST CASE MODULE
package TC_DSM_SecurityAccess_SeedIsZeroWhenSecurityIsGranted;
...
our $PURPOSE = "To check seed bytes response is 0 when security already granted";

#>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>> DOCUMENTATION >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>

=head1 TESTCASE MODULE

TC_DSM_SecurityAccess_SeedIsZeroWhenSecurityIsGranted
...
4. Send <Req_Key> as <level> security access followed by key calculated from seed

5. Again Send <Req_Seed> as <level> security access.

6. Send <Req_Key> as <level> security access followed by key calculated from seed
...
5. Positive response should be obtained with <Seed_Bytes_Zero>

6.The NRC <NRC_RequestSequenceError> should be obtained.
...
=head2 PARAMETER NAMES
	...
	SCALAR 'Session' => 
	SCALAR 'level' => 
	SCALAR 'NRC_RequestSequenceError' => 
	SCALAR 'Seed_Bytes_Zero' => 


=head2 PARAMETER EXAMPLES

	purpose = 'To check request for seed will respond with 0 bytes when already security is granted'
	# input parameters
	...
	# output parameters
	NRC_RequestSequenceError ='0x7F 0x27 0x24'
	Seed_Bytes_Zero = '0x67 0x1 0x0 0x0 0x0 0x0' #hex value
	Req_Seed = '27 01' 
...
my $tcpar_Session;
my $tcpar_level;
my $tcpar_NRC_RequestSequenceError;
my $tcpar_Seed_Bytes_Zero;
my $tcpar_Req_Seed;
my $tcpar_Req_Key;
my $tcpar_Res_Seed;
my $tcpar_Res_Key;
...
sub TC_set_parameters {
	...
	$tcpar_NRC_RequestSequenceError =  GEN_Read_mandatory_testcase_parameter( 'NRC_RequestSequenceError' );
	$tcpar_Seed_Bytes_Zero =  GEN_Read_mandatory_testcase_parameter( 'Seed_Bytes_Zero' );
...
sub TC_stimulation_and_measurement {
	...
	$res_seed1 = _reqres_seed($tcpar_level);
	
	S_teststep("Send '$tcpar_Req_Key' as '$tcpar_level' security access followed by key calculated from seed", 'AUTO_NBR', 'send_req_key_A');			#measurement 2
	$res_key1 = _reqres_key($tcpar_level);
	
	_clearsecurity($tcpar_level);
	S_wait_ms(3000);
	
	S_teststep("Again Send '$tcpar_Req_Seed' as '$tcpar_level' security access.", 'AUTO_NBR', 'again_send_req');			#measurement 3
	$res_seed2 = _reqres_seed($tcpar_level);
	S_w2rep("print $res_seed2", 'green');
	
	S_teststep("Send '$tcpar_Req_Key' as '$tcpar_level' security access followed by key calculated from seed", 'AUTO_NBR', 'send_req_key_B');			#measurement 4
...
sub TC_evaluation {
	...
	S_teststep_detected("positive response should be $res_seed1", 'send_req_seed');
	my @resseed = split(/ /,$res_seed1);
	...
	S_teststep_expected("'$tcpar_Res_Key' should be obtained.", 'send_req_key_A');			#evaluation 2
	S_teststep_detected("positive response should be $res_key1", 'send_req_key_A');
	EVAL_evaluate_string("Evaluating DTC response",$tcpar_Res_Key,$res_key1);

	S_teststep_expected("Positive response should be obtained with '$tcpar_Seed_Bytes_Zero'", 'again_send_req');			#evaluation 3
	S_teststep_detected("positive response should be $res_seed2", 'again_send_req');
	my @resseed1 = split(/ /,$res_seed2);
	if ($tcpar_Res_Seed eq '0x67 0x1 S1 S2 S3 S4'){
		EVAL_evaluate_value("Checking value of byte 1", @resseed1[0],'==',0x67);
		EVAL_evaluate_value("Checking value of byte 2", @resseed1[1],'==',0x1);
	}elsif($tcpar_Res_Seed eq '0x67 0x3 S1 S2 S3 S4'){
		EVAL_evaluate_value("Checking value of byte 1", @resseed1[0],'==',0x67);
		EVAL_evaluate_value("Checking value of byte 2", @resseed1[1],'==',0x3);
	}elsif($tcpar_Res_Seed eq '0x67 0x5 S1 S2 S3 S4'){
		EVAL_evaluate_value("Checking value of byte 1", @resseed1[0],'==',0x67);
		EVAL_evaluate_value("Checking value of byte 2", @resseed1[1],'==',0x5);
	}
	EVAL_evaluate_value("Checking value of byte 3", @resseed1[2],'==',0x00);
	EVAL_evaluate_value("Checking value of byte 4", @resseed1[3],'==',0x00);
	EVAL_evaluate_value("Checking value of byte 5", @resseed1[4],'==',0x00);
	EVAL_evaluate_value("Checking value of byte 6", @resseed1[5],'==',0x00);
	
	S_teststep_expected("The NRC '$tcpar_NRC_RequestSequenceError' should be obtained.", 'send_req_key_B');			#evaluation 4
	S_teststep_detected("Negative response should be $res_key2", 'send_req_key_B');
	EVAL_evaluate_string("Evaluating DTC response",$tcpar_NRC_RequestSequenceError,$res_key2);
	return 1;
}
