#### TEST CASE MODULE
package TC_DSM_Service85_NRC11;
...
our $PURPOSE = "Check NRC11 for service 85";

#>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>> DOCUMENTATION >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>

=head1 TESTCASE MODULE

TC_DSM_Service85_NRC11
...
=head2 PARAMETER NAMES
	...
	SCALAR 'Protocol' => 
	SCALAR 'Session' => 
	SCALAR 'WrongService_CDS' => 
	SCALAR 'NRC_11' => 


=head2 PARAMETER EXAMPLES

	purpose = 'To verify that NRC 11 is received when service request is sent which is not supported'
	
	Addressing_Mode = @('Physical','Functional')
	Protocol = 'UDS' 
	Session = 'Extended' 
	WrongService_CDS = '87 02'
	NRC_11= '7F 87 11'
...
sub TC_set_parameters {
	...
	$tcpar_WrongService_CDS =  GEN_Read_mandatory_testcase_parameter( 'WrongService_CDS' );
...
foreach my $add(@tcpar_Addressing_Mode)
	{	
		...
		S_teststep("Send '$tcpar_WrongService_CDS'.", 'AUTO_NBR', 'send_wrongservice_cds'."_$add");			#measurement 2
		$res{$add} = GDCOM_request($tcpar_WrongService_CDS,$tcpar_NRC_11,'strict');
	}
	return 1;
}
...
foreach my $add(@tcpar_Addressing_Mode)
{
	...
	S_teststep_expected("$add - '$tcpar_NRC_11' should be obtained.", 'send_wrongservice_cds'."_$add");			#evaluation 2
	S_teststep_detected("$add - response is obsered $res{$add}", 'send_wrongservice_cds'."_$add");
	EVAL_evaluate_string("Evaluating DTC response",$tcpar_NRC_11,$res{$add});
}
	return 1;
}
