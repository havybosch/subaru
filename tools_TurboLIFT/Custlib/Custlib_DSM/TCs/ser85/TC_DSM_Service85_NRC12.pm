#### TEST CASE MODULE
package TC_DSM_Service85_NRC12;
...
our $PURPOSE = "to check NRC12 for service85";

#>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>> DOCUMENTATION >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>

=head1 TESTCASE MODULE

TC_DSM_Service85_NRC12
...
=head2 PARAMETER EXAMPLES
	...
	purpose = 'To verify that NRC12 is received when service request is sent which is not supported in Active Session'
	
	Addressing_Mode = @('Physical','Functional') 	
	
	Protocol = 'UDS' 
	
	Session1 = 'ExtendedSession' 
...
foreach my $add(@tcpar_Addressing_Mode)
	{	
		...
		$res{$add} = GDCOM_request($tcpar_REQ_OFF,$tcpar_NRC_12,'strict' );
	}
	return 1;
}
...
foreach my $add(@tcpar_Addressing_Mode)
{
	...
	S_teststep_expected("$add - '$tcpar_NRC_12' should be obtained.", 'send_req_off'."_$add");			#evaluation 2
	S_teststep_detected("$add - response is obsered $res{$add}", 'send_req_off'."_$add");
	EVAL_evaluate_string("Evaluating DTC response",$tcpar_NRC_12,$res{$add});
}
	return 1;
}
