#### TEST CASE MODULE
package TC_DSM_Service1905_ExtraRAM;

#### DONT MODIFY THIS SECTION ####
use strict;
use warnings;

###-------------------------------###
our $VERSION = q$Revision: 1.1 $;
our $HEADER = q$Header: Include/TCpmGenerator/Create_TCpm.pl 1.1 2015/01/21 16:00:45MEZ Geisler Martin (CC-PS/EPS2) (gem1si) develop  $;

#----------------------- TEST SPECIFICATION ------------------------------
#This script is based on TS: <TS name> (e.g. TS_SWM_SwitchMgt)
#TS version in DOORS: <TS version based on which this script is written/updated> (e.g. 3.30)
#-------------------------------------------------------------------------

#### INCLUDE ENGINE MODULES ####

use LIFT_general;
use INCLUDES_Project; #necessary
#include further modules here

##################################

our $PURPOSE = "<summarize what this test is good for>";

#>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>> DOCUMENTATION >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>

=head1 TESTCASE MODULE

TC_DSM_Service1905_ExtraRAM

=head1 PURPOSE

<explain what this test is good for>

=head1 TESTCASE DESCRIPTION


I<B<Initialisation>>

StandardPrepNoFault


I<B<Stimulation and Measurement>>

1. Switch On ECU.

2. Create <Number> of  faults.

3.Send <Req_ReadDTCSnapshotRecord> followed by <RecordNo_1>

4.DO IGN reset.

5.Send <Req_ReadDTCSnapshotRecord> followed by <RecordNo_1>


I<B<Evaluation>>

1.

2. <Number> fault should be in <ActiveState>

3. <PR_ReadDTCSnapshotRecord> should be obtained.

4.-

5.<PR_ReadDTCSnapshotRecord> should be obtained.


I<B<Finalisation>>

Reset/Remove the test condition created in test case


=head1 PARAMETER DESCRIPTION


=head2 PARAMETER NAMES

	SCALAR 'purpose' => 
	SCALAR 'Req_ReadDTC' => 
	SCALAR 'RecordNo_1' => 
	SCALAR 'Number' => 
	SCALAR 'PR_ReadDTCSnapshotRecord' => 
	SCALAR 'ActiveState' => 


=head2 PARAMETER EXAMPLES

	purpose= 'To check response of $19 $05 when BIU660 msg transmission is stopped'
	
	# input parameters
	Req_ReadDTC = 'REQ_<Fetch {Service description}>'  
	RecordNo_1 = '29'
	Number ='33'#33rd fault is extra RAM fault.
	
	# output parameters
	PR_ReadDTCSnapshotRecord = 'PR_ReadDTCInformation_ReportDTCSnapshotRecordByRecordedNumber' 
	ActiveState = 'FilteredState'

=cut



#PARAMETERS
################ Parameters from .par file ###################
my $tcpar_purpose;
my $tcpar_Req_ReadDTC;
my $tcpar_RecordNo_1;
my $tcpar_Number;
my $tcpar_PR_ReadDTCSnapshotRecord;
my $tcpar_ActiveState;

################ global parameter declaration ###################
#add any global variables here


###############################################################

sub TC_set_parameters {

	$tcpar_purpose =  GEN_Read_mandatory_testcase_parameter( 'purpose' );
	$tcpar_Req_ReadDTC =  GEN_Read_mandatory_testcase_parameter( 'Req_ReadDTC' );
	$tcpar_RecordNo_1 =  GEN_Read_mandatory_testcase_parameter( 'RecordNo_1' );
	$tcpar_Number =  GEN_Read_mandatory_testcase_parameter( 'Number' );
	$tcpar_PR_ReadDTCSnapshotRecord =  GEN_Read_mandatory_testcase_parameter( 'PR_ReadDTCSnapshotRecord' );
	$tcpar_ActiveState =  GEN_Read_mandatory_testcase_parameter( 'ActiveState' );

	return 1;
}

sub TC_initialization {

	S_teststep("StandardPrepNoFault", 'AUTO_NBR');
	GEN_StandardPrepNoFault();

	return 1;
}

sub TC_stimulation_and_measurement {

	S_teststep("Switch On ECU.", 'AUTO_NBR');

	S_teststep("Create '$tcpar_Number' of  faults.", 'AUTO_NBR', 'create_number_of');			#measurement 1

	S_teststep("Send '$tcpar_Req_ReadDTCSnapshotRecord' followed by '$tcpar_RecordNo_1'", 'AUTO_NBR', 'send_req_readdtcsnapshotrecord_A');			#measurement 2

	S_teststep("DO IGN reset.", 'AUTO_NBR');

	S_teststep("Send '$tcpar_Req_ReadDTCSnapshotRecord' followed by '$tcpar_RecordNo_1'", 'AUTO_NBR', 'send_req_readdtcsnapshotrecord_B');			#measurement 3

	return 1;
}

sub TC_evaluation {

	S_teststep_expected("'$tcpar_Number' fault should be in '$tcpar_ActiveState'", 'create_number_of');			#evaluation 1
	S_teststep_detected("<<add detected result here>>", 'create_number_of');

	S_teststep_expected("'$tcpar_PR_ReadDTCSnapshotRecord' should be obtained.", 'send_req_readdtcsnapshotrecord_A');			#evaluation 2
	S_teststep_detected("<<add detected result here>>", 'send_req_readdtcsnapshotrecord_A');

	S_teststep_expected("'$tcpar_PR_ReadDTCSnapshotRecord' should be obtained.", 'send_req_readdtcsnapshotrecord_B');			#evaluation 3
	S_teststep_detected("<<add detected result here>>", 'send_req_readdtcsnapshotrecord_B');

	return 1;
}

sub TC_finalization {

	return 1;
}


1;
