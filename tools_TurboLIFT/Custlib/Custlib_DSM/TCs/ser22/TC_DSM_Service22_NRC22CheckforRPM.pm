#### TEST CASE MODULE
package TC_DSM_Service22_NRC22CheckforRPM;
...
our $PURPOSE = "To check NRC22 for Engine RPM";

#>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>> DOCUMENTATION >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>

=head1 TESTCASE MODULE

TC_DSM_Service22_NRC22CheckforRPM
...
=head2 PARAMETER EXAMPLES
	...
	purpose = 'To check NRC22 for Engine RPM and Vehicle Speed' 
	#input parameter
...
foreach my $add(@tcpar_Addressing_Mode)
	{	
		...
		{			
			...
			S_teststep("Send '$tcpar_REQ_EngineRPM'", 'AUTO_NBR', 'send_req_enginerpm'."_$add"."_$ses");			#measurement 2
			$res1{$add}{$ses} = GDCOM_request($tcpar_REQ_EngineRPM,$tcpar_NRC22,'strict');
		}
	}
	return 1;
}
...
{	
	foreach my $ses (@tcpar_Session)
	{
		...
		S_teststep_detected("$ses - Positive response is $res_sess{$add}{$ses}", 'enter_supported_session'."_$add"."_$ses");
		if ($ses =~/default/)
		{
		...
		S_teststep_expected("$ses - '$tcpar_NRC22' shall be obtained.", 'send_req_enginerpm'."_$add"."_$ses");			#evaluation 2
		S_teststep_detected("$ses - DTC response is $res1{$add}{$ses}", 'send_req_enginerpm'."_$add"."_$ses");
...
sub _CheckCondition{
	...
	if($sub_condition eq 'EGI040MsgTimeOut')
	{
		COM_stopMessages('EGI040');
	}elsif($sub_condition eq 'EGI040DLCError')
	{
		CA_set_EnvVar_value('enV_EGI040_DLC',0);	
	}elsif($sub_condition eq 'EGI040ChecksumError')
	{
		CA_set_EnvVar_value('_enV_EGI040_ChksumError',1);
	}elsif($sub_condition eq 'EGI040MsgCounterError')
	{
...
sub _ResetCondition{
	...
	if($sub_condition1 eq 'EGI040MsgTimeOut')
	{
		COM_startMessages('EGI040');
	}elsif($sub_condition1 eq 'EGI040DLCError')
	{
		CA_set_EnvVar_value('enV_EGI040_DLC',8);	
	}elsif($sub_condition1 eq 'EGI040ChecksumError')
	{
		CA_set_EnvVar_value('_enV_EGI040_ChksumError',0);
	}elsif($sub_condition1 eq 'EGI040MsgCounterError')
	{
