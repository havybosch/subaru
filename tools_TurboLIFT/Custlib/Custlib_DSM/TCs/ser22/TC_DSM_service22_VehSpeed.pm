#### TEST CASE MODULE
package TC_DSM_service22_VehSpeed;
...
our $PURPOSE = "To verify the response when different ranges of VehicleSpeed is given to VDC139 Signal";

#>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>> DOCUMENTATION >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>

=head1 TESTCASE MODULE

TC_DSM_service22_VehSpeed
...
4. Send request <REQ_VehicleSpeed>.
...
4.Positive Response <PR_VehicleSpeed> shall be obtained along with <Data_Length> 
...
=head2 PARAMETER EXAMPLES
	...
	purpose = 'To verify the response when different ranges of VehicleSpeed is given to VDC139 Signal'
	#input parameter
	Session = @('default','extended')
	Addressing_Mode =@('Physical','Functional')
	REQ_VehicleSpeed = '22 10 1A'
	Signal = 'VDC139_2_0_SPD_WHEEL_AVE'
	#output parameter
	Data_Length = '1'
	PR_VehicleSpeed = '62 10 1A 00'
	DataValue  = '0'#km/h 
...
sub TC_set_parameters {
	...
	$tcpar_PR_VehicleSpeed =  GEN_Read_mandatory_testcase_parameter( 'PR_VehicleSpeed' );
...
sub TC_stimulation_and_measurement {
	...
	{	
		...
		{			
			...
			S_teststep("Send request '$tcpar_REQ_VehicleSpeed'.", 'AUTO_NBR', 'send_request_req'."_$add"."_$ses");			#measurement 2
			$res1{$add}{$ses} = GDCOM_request($tcpar_REQ_VehicleSpeed,$tcpar_PR_VehicleSpeed,'strict');
		}
	}
	return 1;
}
...
{	
	foreach my $ses (@tcpar_Session)
	{
		...
		S_teststep_expected("$ses - Positive Response '$tcpar_PR_VehicleSpeed' shall be obtained along with '$tcpar_Data_Length' ", 'send_request_req'."_$add"."_$ses");			#evaluation 2
		S_teststep_detected("$ses - response is obsered $res1{$add}{$ses}", 'send_request_req'."_$add"."_$ses");
		EVAL_evaluate_string("Evaluating DTC response",$tcpar_PR_VehicleSpeed,$res1{$add}{$ses});
