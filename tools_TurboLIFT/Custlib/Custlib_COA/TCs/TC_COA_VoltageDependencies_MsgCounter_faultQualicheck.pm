#### TEST CASE MODULE
package TC_COA_VoltageDependencies_MsgCounter_faultQualicheck;
...
our $HEADER = q$Header: TCs/TCs_Project/CAN/TC_COA_VoltageDependencies_MsgCounter_faultQualicheck.pm 1.1 2016/08/26 16:14:34ICT pnv5kor develop  $;
...
TC_COA_VoltageDependencies_MsgCounter_faultQualicheck
...
3.Wait for <t_wait> msec with <tolerance>.
...
7.Wait for <t_wait> msec with <tolerance>.
...
sub TC_stimulation_and_measurement {
	...
	S_teststep("Wait for '$tcpar_t_wait' msec with '$tcpar_tolerance'.", 'AUTO_NBR');
	S_wait_ms($tcpar_t_wait);
	PD_ReadFaultMemory();
...
sub TC_finalization {
	$CAN_Mapping = S_get_contents_of_hash( ['Mapping_CAN'] );
	my $cntVarName1 = $CAN_Mapping->{'CAN_MESSAGES'}{$tcpar_MSGID}{'CANOE_CNT'};
