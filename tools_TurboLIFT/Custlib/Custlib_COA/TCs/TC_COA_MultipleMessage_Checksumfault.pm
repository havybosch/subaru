#### TEST CASE MODULE
package TC_COA_MultipleMessage_Checksumfault;
...
our $PURPOSE = "To check the checksum fault status when sending multiple messages";

#>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>> DOCUMENTATION >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>

=head1 TESTCASE MODULE

TC_COA_MultipleMessage_Checksumfault
...
=head2 PARAMETER EXAMPLES
	...
	purpose = 'To check the fault qualification/Disqualification ,when the fault present /not present in multiple message from same ECU' 
	...
	Fault='rb_coa_<Test Heading 3>RxMsgChecksumError_flt'
	...
	ReadDTCResp_dequali = '59 02 0A C4 01 00 08' #fault Qualified
...
sub TC_stimulation_and_measurement {
	...
	my $csVarName3 = $CAN_Mapping->{'CAN_MESSAGES'}{$tcpar_MSGID1}{'CANOE_CS'};
	...
	my $csVarName4 = $CAN_Mapping->{'CAN_MESSAGES'}{$tcpar_MSGID2}{'CANOE_CS'};
	CA_set_EnvVar_value($csVarName4 ,0);
	PD_ReadFaultMemory();
