#### TEST CASE MODULE
package TC_COA_MultipleMessage_FaultQuali_check;
...
our $PURPOSE = "To check the fault qualification when the fault is present in one message and not in other message of same ECU";

#>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>> DOCUMENTATION >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>

=head1 TESTCASE MODULE

TC_COA_MultipleMessage_FaultQuali_check
