#### TEST CASE MODULE
package TC_COA_MsgCounterFaultQualicheck_Sporadic;
...
our $HEADER = q$Header: TCs/TCs_Project/CAN/TC_COA_MsgCounterFaultQualicheck_Sporadic.pm 1.1 2016/08/26 16:14:32ICT pnv5kor develop  $;
...
our $PURPOSE = "To check the message counter fault qualification check when message is transmitted sporadically";

#>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>> DOCUMENTATION >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>

=head1 TESTCASE MODULE

TC_COA_MsgCounterFaultQualicheck_Sporadic
...
6.Create <fault> by stop transmission of <MSGID>.
