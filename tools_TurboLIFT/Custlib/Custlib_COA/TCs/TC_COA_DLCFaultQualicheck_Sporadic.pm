#### TEST CASE MODULE
package TC_COA_DLCFaultQualicheck_Sporadic;
...
our $HEADER  = q$Header: TCs/TCs_Project/CAN/TC_COA_DLCFaultQualicheck_Sporadic.pm 1.1 2016/08/26 16:14:30ICT pnv5kor develop  $;
...
our $PURPOSE = "To check the DLC fault qualification check when message is transmitted sporadically";

#>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>> DOCUMENTATION >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>

=head1 TESTCASE MODULE

TC_COA_DLCFaultQualicheck_Sporadic
...
2.Create <fault> by changing the DLC less than 8 on <MSGID>.

3.Wait for <t_wait> msec 

4.Remove <fault> on <MSGID>

5.Create <fault> by changing the DLC less than 8 on <MSGID>.

6.Wait for <t_wait> msec 

7.Send <Req_readDTC> with status mask is FF.

8.Wait for <t_Quali1> msec with <tolerance>.
...
sub TC_stimulation_and_measurement {
	...
	#my $request        = $RequestDetails->{'Request'} . " " ."FF";
	$res1 = GDCOM_request( $tcpar_Req_readDTC, $tcpar_ReadDTCResp, 'strict' );

	S_teststep( "Wait for '$tcpar_t_Quali1' msec with status mask is FF.", 'AUTO_NBR' );
	S_wait_ms( $tcpar_t_Quali1 + $tcpar_tolerance );
