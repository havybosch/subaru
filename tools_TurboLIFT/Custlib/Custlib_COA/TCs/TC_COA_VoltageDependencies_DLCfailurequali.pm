#### TEST CASE MODULE
package TC_COA_VoltageDependencies_DLCfailurequali;
...
our $PURPOSE = "To check the DLC failure fault qualification diagnosis dependency on predefined threshold voltage";

#>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>> DOCUMENTATION >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>

=head1 TESTCASE MODULE

TC_COA_VoltageDependencies_DLCfailurequali
...
=head2 PARAMETER EXAMPLES
	...
	purpose = 'To check the DLC failure fault qualification diagnosis dependency on predefined threshold voltage' 
	# input parameters
...
sub TC_finalization {
	$CAN_Mapping = S_get_contents_of_hash( ['Mapping_CAN'] );
	my $DLCVarName1 = $CAN_Mapping->{'CAN_MESSAGES'}{$tcpar_MSGID}{'CANOE_DLC'};
