#### TEST CASE MODULE
package TC_COA_MultipleMessage_DLCFaultQuali;
...
our $PURPOSE = "To check DLC fault when multiple messages sending in same ECU";

#>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>> DOCUMENTATION >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>

=head1 TESTCASE MODULE

TC_COA_MultipleMessage_DLCFaultQuali
...
=head2 PARAMETER EXAMPLES
	...
	Status_Mask = 'FF'
	Fault='rb_coa_<Test Heading 3>RxMsgTimeout_flt'
	t_Quali = '400' #msec
	t_Dequali = '800' #msec
	ReadDTCResp = '59 02 0A C1 00 00 0A'#fault qualified
	ReadDTCResp_dequali = '59 02 0A C1 00 00 08' #fault dequalified
...
sub TC_stimulation_and_measurement {
	...
	$DLCVarName1 = $CAN_Mapping->{'CAN_MESSAGES'}{$tcpar_MSGID1}{'CANOE_DLC'};
	CA_set_EnvVar_value($DLCVarName1,0);	
	...
	$DLCVarName2 = $CAN_Mapping->{'CAN_MESSAGES'}{$tcpar_MSGID2}{'CANOE_DLC'};
	CA_set_EnvVar_value($DLCVarName2,0);
	...
	CA_set_EnvVar_value($DLCVarName1,8);
	
	S_teststep("Wait for  '$tcpar_t_Dequali'  msec", 'AUTO_NBR');
